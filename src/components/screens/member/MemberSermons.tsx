...
import { CACICard, CACISkeleton, EmptyState, CACIInput } from "@/components/caci/ui";
      ...
      <div className="px-4 py-4 md:px-8 md:py-6 max-w-md mx-auto md:max-w-5xl">
        ...
        {!loading && filtered.length > 0 && (
          <div className="space-y-3">
            {filtered.map((sermon) => (
              <CACICard key={sermon.id} padding="none" hover onClick={() => openSermon(sermon.id)} className="cursor-pointer overflow-hidden">
                <div className="p-4">
                  <div className="flex items-center justify-between gap-3 mb-2">
                    <span className="inline-flex items-center gap-1 text-[12px] text-caci-blue">
                      <Calendar size={11} />
                  ...
                  {sermon.summary && <p className="mt-2 text-[13px] text-n600 line-clamp-2">{sermon.summary}</p>}
                </div>
              </CACICard>
            ))}
          </div>
